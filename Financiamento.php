<?php
  ...
  class Financiamento{
    ...
    private function calcularFatorAcumuladoCapitalizacao($juros,$quantidadePrestacoes){
        $fatorAcumuladoCapitalizacao = ($juros + 1) ^ $quantidadePrestacoes;
        return $fatorAcumuladoCapitalizacao;
    }
    ...
    // é 0.060434457
    private function calcularCoeficientePrice($fatorAcumuladoCapitalizacao,$juros){
        $CoeficientePrice = ($fatorAcumuladoCapitalizacao * $juros) / ($fatorAcumuladoCapitalizacao - 1);
        return $CoeficientePrice;
    ...
    private function calcularValorPrestacao($valorFinanciado,$CoeficientePrice){
        $valorPrestacao = $valorFinanciado * $CoeficientePrice;
    ...
    private function criarParcela($saldoDevedor, $valorPrestacao,$juros){
      $parcela = new Parcela;
      $parcela->valor = $valorPrestacao;
      $parcela->juros = $juros * $saldoDevedor;
      $parcela->valorAmortizacao = $valorPrestacao - $juros;
      $parcela->saldoDevedorAtual = $saldoDevedor - ($valorPrestacao - $juros);
    ...
    public function gerarDemonstrativoPagamento($saldoDevedor, $quantidadePrestacoes,$valorParcela){
      $cont = 0;
      $parcelas = array();
      while($cont < $quantidadePrestacoes):
        $parcelas[$cont] = $saldoDevedor - $valorParcela;
